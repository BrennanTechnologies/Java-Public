...
public class ArrayBackwards {
    public static void main (String[] args) {
        ...
        for (int i = arr.length - 1; i >= 0; i--) {                 // i = 9, 8, 7, 6, 5, 4, 3, 2, 1, 0
            bArr[(arr.length-1)-i] += arr[i];                       // add the value to arr 'backwards' to the new array
        }

        // Start at the end of the 'original' array and work backwards
        for (int i = arr.length - 1; i >= 0; i--) {                 // i = 9, 8, 7, 6, 5, 4, 3, 2, 1, 0

            //------------------------------------------------------
            // ** The BackZinator (TM) (TradeMark & Patent Pending) **
            //------------------------------------------------------
            //  Copy the value from the 'original' array 'backwards' into the 'new' array
            //      - i.e. bArr[i] = arr[i]
            //
            //    *** Warning: Unlicensed Use is Prohibited. ***

            bArr[(arr.length-1)-i] += arr[i];                       // This is a BackZinator (TM)
                                                                    // add the value to arr 'backwards' to the new array
        }
